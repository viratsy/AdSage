    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => toggleFav.mutate()} className="p-2 rounded-lg hover:bg-white/5 transition-colors" aria-label="Toggle favorite">
            <Heart size={16} className={ad.favorite ? 'text-red-400 fill-red-400' : 'text-gray-400'} />
          </button>
          <button onClick={() => { if (confirm('Delete this ad?')) deleteAd.mutate(); }} className="p-2 rounded-lg hover:bg-white/5 transition-colors" aria-label="Delete ad">
        ...
        <div className="space-y-4">
          {!ai_analysis ? (
            <div className="rounded-xl p-6 text-center space-y-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
              ...
              {triggerAi.isError && (
                <p className="text-xs text-red-400">{(triggerAi.error as any)?.response?.data?.error || 'Failed'}</p>
              )}
            </div>
          ) : (
            <>
              ...
              <div className="rounded-xl p-5 grid grid-cols-2 gap-3" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
                {[['Hook Type', ai_analysis.hook_type], ['Emotion', ai_analysis.emotional_trigger],
                  ...
                  ['CTA Strength', ai_analysis.cta_strength]].map(([label, value]) => (
                  <div key={label}>
                  ...
                  <ul className="space-y-2">
                    {ai_analysis.generated_hooks.map((hook: string, i: number) => (
                      <li key={i} className="flex items-start justify-between gap-2 group">
                        ...
                        <button onClick={() => copyText(hook, `hook-${i}`)} className="shrink-0 opacity-0 group-hover:opacity-100" aria-label="Copy">
                ...
                <div className="rounded-xl p-5" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {ai_analysis.generated_ctas.map((cta: string, i: number) => (
                      <button key={i} onClick={() => copyText(cta, `cta-${i}`)}
                        className="text-xs px-3 py-1.5 rounded-full transition-colors"
                        style={{ background: copied === `cta-${i}` ? 'rgba(34,197,94,0.2)' : 'var(--surface-2)',
                          color: copied === `cta-${i}` ? '#22c55e' : 'var(--text-muted)', border: '1px solid var(--border)' }}>
                        {cta}
                      </button>
                    ))}
                  </div>
